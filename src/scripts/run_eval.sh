...
for (( i=0; i<6; i++)); do
  ...
  loadname=../checkpoints/$dataset/$PRUNE_STR/$RUN_TAG'_pruned_final.pt'

  ### This is "if ($loadname doesn't exist)"
  if [ ! -f $loadname ]; 
    then
      echo 'Final file not found! Using postprune'
      loadname=../checkpoints/$dataset/$PRUNE_STR/$RUN_TAG'_pruned_postprune.pt'
  
      if [ ! -f $loadname ]; 
        then
          echo 'Postprune file not found! Using preprune'
          loadname=../checkpoints/$dataset/$PRUNE_STR/$RUN_TAG'.pt'
          preprune=1
      fi
  fi
